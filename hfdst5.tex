...
cumulatieve afstanden ter beschikking zijn. Het voordeel dat dit model heeft
ten opzichte van de volgende modellen is dat er geen \ac{gps}-data nodig is.
...
\begin{table}[h]
    ...
    \captionsetup{justification=centering}
    \caption{Aanval op basis van ruwe gps-locaties (geen smoothing) en snelheid}\label{tab:noSmoothing}
\end{table}
...
\acp{EPZ} tonen de metrieken veel minder verval. Ook andere metrieken dan
success rate vertonen een gelijkaardig patroon, waarbij de afzwakking veel
minder sterk is dan bij het gebruik van ruwe \ac{gps}-punten. Dit toont dat het
gebruik van smoothing zeker een significante toegevoegde waarde heeft. We zien
echter wel dat over het algemeen de accuracy iets hoger ligt, wat wel nog duidt
op onzekere voorspellingen. De fouten in de \ac{gps}-locaties spelen dus wel
nog zeker een rol in de voorspelling.
\begin{table}[h]
    \centering
    \scalebox{0.5}{
    ...
    \captionsetup{justification=centering}
    \caption{Aanval op basis van gesmoothe gps-data en snelheid, met een empirisch bepaald optimaal smoothing window $n=100$}\label{tab:optimal_smoothing}
\end{table}
