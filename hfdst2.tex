...
Rechts bovenaan zijn de algemene basisstatistieken te zien. Deze zijn de totale
...
twee meest waarschijnlijke technieken zijn \textit{Map Snapping} (ook wel
\textit{Map Matching} genoemd) en \textit{Smoothing}.
...
staan meestal drie opties, \textit{zichtbaar voor iedereen}, \textit{zichtbaar
    voor volgers} en \textit{zichtbaar voor niemand}. Er kan ook zelf een keuze
...
doeltreffendheid van \acp{EPZ} bij fitnesstrackers. Hassan et al.\ beschreven
in 2018 een implementatie van EPZ waarbij het centrum van de zone de gevoelige
...
van~\citeauthor{sec18has3:online} wordt gefocust op de reconstructie van de
cirkel op basis van 3 punten op de rand, wat te zien is op
...
een gebruiker verhoogt. Ze beschrijven een inferentie aanval die gebruikmaakt
van de totale afstand die terug te vinden bij de activiteit. Het principe van
deze inferentie aanval wordt uitvoerig beschreven in
...
al.\ niet feilloos zijn, en dat ze deze kunnen omzeilen met een successrate van
...
\citeauthor{Verdonck_2022}~\cite{Verdonck_2022}. Deze thesis bouwt in grote
mate verder op de paper van \citeauthor{Dhondt}, maar maakt gebruik van
...
stratenplan kan hij via de gekende hoogteverschillen een inferentie aanval
